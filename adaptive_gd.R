...
L_GP <- function(wgp,pi_gp,U3,S3,l,Y,s,xte,xtr,Rinv,epsilon=1e-8) {		
...
Adam_one_obs_only <- function(wgp,pi_gp,U3,S3,Y,S,xte_vec,xtr_vec,Rinv_lst,xstar,r_v,l,tao,e,miter,emiter) {
	...
	for (i in 1:miter) {
		...
		loglik = sum(unlist(mclapply(1:N, function(x) L_GP(wgp[x],pi_gp,U3,S3,l,Y[x,-xstar][r_v[x,]],S[x],xte_vec[x],xtr_vec[x,][r_v[x,]],Rinv_lst[[x]]), mc.cores=num_cores)))
