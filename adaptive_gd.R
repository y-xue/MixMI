...
L_GP <- function(wgp,pi_gp,l,Y,s,xte,xtr,Rinv,epsilon=1e-8) {		
	if (length(Y) == 0 || length(unique(Y)) == 1) {
		L2 = pi_gp
		loglik = wgp * log_with_limits(L2,epsilon)
	}
	else {
		gp_sigma2 <- sig2(l,Y,xtr,Rinv=Rinv)
		m <- yhat(l,xte,xtr,Y,Rinv=Rinv)
		k <- s2(l,gp_sigma2,xte,xtr,Y,Rinv=Rinv)
...
Adam_one_obs_only <- function(wgp,pi_gp,Y,S,xte_vec,xtr_vec,Rinv_lst,xstar,r_v,l,tao,e,miter,emiter) {
	N = length(S)
	beta1 = 0.9
	beta2 = 0.999
	m = 0
	...
	for (i in 1:miter) {
		...
		loglik = sum(unlist(mclapply(1:N, function(x) L_GP(wgp[x],pi_gp,l,Y[x,-xstar][r_v[x,]],S[x],xte_vec[x],xtr_vec[x,][r_v[x,]],Rinv_lst[[x]]), mc.cores=num_cores)))
